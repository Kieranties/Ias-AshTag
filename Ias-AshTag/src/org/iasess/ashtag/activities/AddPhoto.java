...
public class AddPhoto extends InvadrActivityBase {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (_package == null) {
			...
			if (extras != null && extras.containsKey(Intent.EXTRA_STREAM)) {
				...
				// need to set a flag to track that we have come in from
				// external source
	...
	/**
	 * Populates the {@link ImageView} in this activity with the image from the
	 * given {@link URI}
	 * 
	 * @param uri
	 *            The {@link URI} to be displayed
	 */
	private void setImageView() {
		if (_package != null) {
			ImageView iv = (ImageView)findViewById(R.id.imageView);
			String imagePath = _package.getImagePath();
			DisplayImageOptions displayOptions = new DisplayImageOptions.Builder().cacheInMemory()
