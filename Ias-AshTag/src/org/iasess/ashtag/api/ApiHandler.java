...
public class ApiHandler {
	 ...
	 * 
	 * @param img
	 *            The physical path to an image to be submitted
	 * @param taxa
	 *            The primary key identity for the sighting
	 * @param lat
	 *            The latitude of the sighting
	 * @param lon
	 *            The longitude of the sighting
	 * @param user
	 *            The user performing the submission
	...
	public static SubmissionResponse submitSighting(SubmitParcel submitPackage) {
		try {
			...
			HttpHandler.executeMultipartPost(url, submitPackage.getSubmitContent());
			new Gson();
			return new SubmissionResponse(); // gson.fromJson(resp,
