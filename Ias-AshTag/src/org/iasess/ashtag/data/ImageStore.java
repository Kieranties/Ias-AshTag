...
public class ImageStore extends IasDatabase {
	 ...
	 */
	private static final String TABLE_CREATE = "CREATE TABLE " + TABLE_NAME + " ( " + COL_PK + " int primary key,"
												+ COL_TAXON_ITEM_ID + " int, " + COL_SIZE + " text, " + COL_URI
												+ " text);";
	...
	public Cursor getByTaxaId(long id, String size) {
		return executeStringQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + COL_TAXON_ITEM_ID + " = " + id + " AND "
									+ COL_SIZE + " = " + size);
	}
	...
	public void update(Map<String, String> collection, long id, SQLiteDatabase db) {
		for (Entry<String, String> entry : collection.entrySet()) {
			...
			db.delete(TABLE_NAME, COL_TAXON_ITEM_ID + "=? AND " + COL_SIZE + "=?", new String[] { String.valueOf(id),
				size });
